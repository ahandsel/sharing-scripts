...
#  directory: The directory to search for markdown files. (default: current working directory)
#  config_file: The markdownlint configuration file. (optional)
#
# Version: 1.3
#  - v1.3: Fixed "curly quotes" replacement function to label every file.
#  - v1.2: Added function to replace curly quotes with straight quotes.
...
function replace_curly_quotes {
  local file_path="$1"
  local quotes_replaced=false

  if grep -q '[‘’“”]' "$file_path"; then
    sed -i.bak -e "s/‘/'/g" -e "s/’/'/g" -e 's/“/"/g' -e 's/”/"/g' "$file_path" && rm "$file_path.bak"
    if [ $? -eq 0 ]; then
      quotes_replaced=true
    fi
  fi

  if [ "$quotes_replaced" = true ]; then
    printf "%s: curly quotes replaced\n" "$file_path" >>$LOG_FILE
  fi
}
